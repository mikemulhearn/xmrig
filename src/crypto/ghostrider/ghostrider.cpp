...
void benchmark()
{
    ...
    std::thread t([]() {
        ...
        printf("Running GhostRider benchmark on logical CPUs %u and %u (max scratchpad size %zu MB, huge pages %s)\n", thread_index1, thread_index2, max_scratchpad_size >> 20, memory->isHugePages() ? "on" : "off");
        ...
        for (uint32_t algo = 0; algo < 6; ++algo) {
            for (uint64_t step : { 1, 2, 4}) {
                ...
                for (uint32_t iter = 0;; ++iter) {
                    ...
                    // Stop after 10 milliseconds, but only if at least 10 iterations were done
                    if ((iter >= 10) && (duration_cast<milliseconds>(t1 - start_time).count() >= 10)) {
                        break;
                    }
                ...
                printf("%24s | %" PRIu64 "x1 | % .2f h / s\n", cn_names[algo], step, hashrate);
        ...
        for (uint32_t algo = 0; algo < 6; ++algo) {
            for (uint64_t step : { 1, 2, 4}) {
                ...
                for (uint32_t iter = 0;; ++iter) {
                    ...
                    // Stop after 20 milliseconds, but only if at least 10 iterations were done
                    if ((iter >= 10) && (duration_cast<milliseconds>(t1 - start_time).count() >= 20)) {
                        break;
                    }
                ...
                const double hashrate = step * 2.0 / min_dt;
                printf("%24s | %" PRIu64 "x2 | %.2f h/s\n", cn_names[algo], step, hashrate);
    ...
    for (int algo = 0; algo < 6; ++algo) {
        printf("%24s %ux%u - %.2f h/s\n", cn_names[algo], tuneDefault[algo].step, tuneDefault[algo].threads, tuneDefault[algo].hashrate);
        if ((tune8MB[algo].step != tuneDefault[algo].step) || (tune8MB[algo].threads != tuneDefault[algo].threads)) {
            printf("%24s %ux%u - %.2f h/s\n", cn_names[algo], tune8MB[algo].step, tune8MB[algo].threads, tune8MB[algo].hashrate);
        }
    }
}
...
void hash_octa(const uint8_t* data, size_t size, uint8_t* output, cryptonight_ctx** ctx, HelperThread* helper)
{
    ...
    if (memcmp(cn_indices, prev_indices, sizeof(prev_indices)) != 0) {
        ...
        for (int i = 0; i < 3; ++i) {
            printf("%s | ", cn_names[cn_indices[i]]);
        }
        printf("\n");
    }
